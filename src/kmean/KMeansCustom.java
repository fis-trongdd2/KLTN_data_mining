 ...
 * 
 */
public class KMeansCustom implements Clusterer {
    /**
     ...
     */
    public KMeansCustom(int clusters, int iterations) {
     ...
     */
    public KMeansCustom(int clusters, int iterations, DistanceMeasure dm) {
    ...
    /**
     * Execute the KMeansCustom clustering algorithm on the data set that is provided.
     * 
...
//            }
        	int tmp;
        	do{
        		tmp = rg.nextInt(data.size());
        	}while(listRandom.contains(tmp));
            this.centroids[j] = data.get(tmp);
        }
